...
game<-function(board1,board2){
  ...
  if(sum(en$board1[,,2]==3)==20){
    
    winner2="You!"
    
  }else{
    if(sum(ai$board2[,,2]==3)==20){
      winner2="Very bad AI :("
    }
  }
          
  message("GAME")  
  rmarkdown::render("game_summary.Rmd", params = list(
    winner = winner2,
    shots = en$player_shots,
    accuracy = round(100*en$player_hits/en$player_shots,0)
  ))
  openHTML <- function(x) browseURL(paste0('file:/', file.path(getwd(), x)))
  openHTML("game_summary.html")
  browseURL(file.path('file:/', getwd(), "game_summary.html"))
}
